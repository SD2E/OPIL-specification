...
An \opil{ExecutionRequest} specifies a particular set of \opil{ParameterValue}s, \opil{SampleSet}s, and \opil{Measurement}s to be carried out. 
A \opil{ProtocolInterface} specifies which \opil{ExecutionRequest}s are allowed for a given protocol with a parallel construction of \opil{Parameter}s, \opil{SampleSet}s, and \opil{MeasurementType}s
...
\end{quote}
These specifies could be encoded into an \opil{ExperimentRequest} by dividing them up as follows:
\begin{itemize}
