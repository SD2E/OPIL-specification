...
\todo[inline]{Delete the required field?}
...
For example, it would be good to use a \opil{Parameter} to indicate whether an optional post-culturing dilution step should be used.
It would be problematic to use a \opil{Parameter} to indicate which culture media should be used (which can be represented well via \opil{SampleSet}) or to indicate whether samples should be evaluated with an optional proteomics step (which can be represented well via \opil{Measurement}).
